...
is a unique vector $v\in V$ such that $T(u)=\langle u,v\rangle $for every $u\in V$.
	...
	\end{theorem}
	\begin{corollary}
		A symmetric real matrix $A$ is positive definite if and only if all of its eigenvalues are positive. (Note that when $A$ is not necessary symmetric, being positive definite implies having positive eigenvalues, but the converse is not always true).
	\end{corollary}
	\begin{definition}
...
\begin{definition}
	The singular values of a general (non-square) matrix are the square roots of the Gram matrix $K=A^tA$.
\end{definition}
\begin{theorem}
...
\end{theorem}
\begin{remark}
	If $A$ has no zero eigenvalue, then the SVD of $A$ is `the same' as the spectral decomposition of $K=A^tA$:
