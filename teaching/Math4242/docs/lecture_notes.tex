...
%	\end{proposition}
\begin{proposition}
	Let $K=(k_{ij})$ be the matrix whose entries are the inner product of the basis vectors, i.e. $k_{ij}=\langle e_i,e_j\rangle$. Then for $x=(x_1,\cdots,x_n)$ and $y=(y_1,\cdots,y_n)$ we have
	\[\langle x,y\rangle=\langle \sum x_ie_i,\sum y_ie_i\rangle = \sum_{i,j} x_iy_j \langle e_i,e_j\rangle=x^tKy\]
\end{proposition}
\begin{definition}
...
Every inner product is given by $\langle x,y\rangle =x^t Ky$ where $K$ is a positive-definite matrix.
	...
	\end{corollary}
	\begin{theorem}
		Given any basis $w_1,\cdots,w_n$ of $V$, one can construct an orthogonal basis $v_1,\cdots,v_n$ using the Gram-Schmit process:
		\begin{itemize}
			\item $v_1=w_1$.
