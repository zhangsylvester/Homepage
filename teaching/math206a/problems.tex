...
\end{problem}
	...
	{\color{red}Submit any 4 of the following}
	\begin{problem}[2+]
	Find all representations of $D_{2n}$ up to equivalence. (Hint: $C_n$ is a normal subgroup of $D_{2n}$).
\end{problem}

\begin{problem}[2]
	(1) For any $w\in S_n$, prove that $\chi^{(n-1,1)}(w)=\#\{\text{fixed points of }\pi\}-1$. 
	
	(2) Compute the Specht module for $\lambda=(n-1,1)$. 
\end{problem}
	\begin{problem}[2+]
...
\begin{problem}[2]
	Write down the character table of $S_5$.
\end{problem}
\begin{problem}[3-]
	What's the determinant of the character table of $S_n$? 
\end{problem}

\begin{problem}[3]
The Durfee square of a partition is the largest square contained in $\lambda$.
	Show that if $\chi_\lambda(w)=0$ whenever the side length of the Durfee square is larger than the number of cycles of $w$.
\end{problem}
