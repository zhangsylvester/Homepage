...
class GrowthDiagram:
    ...
    def _place_dots(self):
        self.dots = [(i+1, a - self.B + 1) for i, a in enumerate(self.L)]
    ...
    def auto_color_left_sequence(self):
        ...
        for i, color in enumerate(self.color_list):
            ...
            while True:
                ...
                for sidx in self.comps[cid]: self.seg_color[sidx] = color
                x += 1
    ...
    def display_colored_shadow(self, scale=1.0):
        ...
        xs = [c-0.5 for c,r in self.dots]; ys = [r-0.5 for c,r in self.dots]
        ax.scatter(xs, ys, s=30*scale, color='blue', zorder=3)
        for idx, seg in enumerate(self.segments):
            col = self.seg_color.get(idx, 'black'); lw = 2 if idx in self.seg_color else 1
            if seg['type']=='line':
                (x1,y1),(x2,y2) = seg['points']; ax.plot([x1,x2],[y1,y2], color=col, lw=lw, zorder=2)
            else:
                cx,cy = seg['center']; arc = Arc((cx, cy),1,1,theta1=seg['theta1'],theta2=seg['theta2'],color=col,lw=lw,zorder=2)
                ax.add_patch(arc)
        ax.invert_yaxis(); ax.axis('off'); plt.show()

    def display_edge_labels(self, scale=1.0):
        """
        Draw a dotted growth diagram grid, show uncolored shadow lines in gray, and annotate each edge label.
        """
        fig, ax = plt.subplots(figsize=(10*scale,10*scale))
        ax.set_aspect('equal')
        # draw dotted grid lines
        for x in range(self.n+1):
            ax.plot([x, x], [0, self.num_rows], linestyle=':', color='black', lw=0.5)
        for y in range(self.num_rows+1):
            ax.plot([0, self.n], [y, y], linestyle=':', color='black', lw=0.5)
        ...
        for seg in self.segments:
            if seg['type'] == 'line':
                ...
                ax.add_patch(arc)
        # annotate horizontal edges with thick labels on lines
        for r in range(self.num_rows+1):
            for c in range(self.n):
                label = self.H[r][c]
                ax.text(c+0.5, r, str(label), ha='center', va='center',
                        fontsize=10, fontweight='bold', color='black')
        # annotate vertical edges with thick labels on lines
        for r in range(self.num_rows):
            for c in range(self.n+1):
                label = self.V[r][c]
                ax.text(c, r+0.5, str(label), ha='center', va='center',
                        fontsize=10, fontweight='bold', color='black')
        # plot dots
        xs = [c-0.5 for c, r in self.dots]
        ys = [r-0.5 for c, r in self.dots]
        ax.scatter(xs, ys, s=30*scale, color='blue', zorder=2)
        ax.invert_yaxis()
        ax.axis('off')
        plt.show()
